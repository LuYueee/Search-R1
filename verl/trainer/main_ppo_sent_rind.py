    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if valid_response_length > 0:
                ...
                last_step_val = float(reward_tensor[i, last_pos].item())
        ...
        S_ep = float(np.sum(s_list))
        SS_ep = float(np.sum(s_list ** 2))
        Cross = float(np.sum(2.0 * v_last * s_list))
        ...
        if N_total > 0:
            ...
            for v, s in zip(v_last, s_list):
                ...
                if vs > 0:
                    ...
                    neg_dec += 1
            pos_rate = pos_dec / N_total
            neg_rate = neg_dec / N_total
        else:
