...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        # accumulators for efficient sentence-level statistics
        S_ep = 0.0
        SS_ep = 0.0
        Cross = 0.0
        pos_sent_dec_sum = 0
        ...
        for i in range(len(data)):
            ...
            fn_list.append(fn)
            dec_cnt = tp + fp + tn + fn
            decision_counts.append(dec_cnt)
            ...
            if valid_response_length > 0:
                ...
                v_last = float(reward_tensor[i, last_pos].item())
                ...
                if vs > eps:
                    ...
                    neg_sent += 1
                if abs(v_last) <= eps and abs(vs) > eps:
                    dec_cnt += 1
                elif abs(v_last) > eps and abs(vs) <= eps:
                    dec_cnt = max(0, dec_cnt - 1)
                reward_tensor[i, last_pos] = reward_tensor[i, last_pos] + s
                all_scores.append(score)

            pos_sent_dec_sum += pos_sent
            neg_sent_dec_sum += neg_sent
            pos_sent_all_sum += pos_sent
            neg_sent_all_sum += neg_sent
        ...
        if valid_vals.numel() > 0:
            pos_rate_token = float((valid_vals > 0).float().mean().item())
            neg_rate_token = float((valid_vals < 0).float().mean().item())
            step_min = float(valid_vals.min().item())
            step_max = float(valid_vals.max().item())
        else:
            pos_rate_token = neg_rate_token = 0.0
            step_min = step_max = 0.0

        print(
            f"[DEBUG][Step] step_reward/mean: {step_mean:.4f}, /std: {step_std:.4f}, "
            f"/min: {step_min:.4f}, /max: {step_max:.4f}, "
            f"/pos_rate(token): {pos_rate_token:.4f}, /neg_rate(token): {neg_rate_token:.4f}"
        )
        ...
        print(
            "[DEBUG][Step][Sentence] "
            f"pos_rate(decision): {sdiv(pos_sent_dec_sum, m_decision_after):.4f}, "
            f"neg_rate(decision): {sdiv(neg_sent_dec_sum, m_decision_after):.4f}, "
            f"pos_rate(all): {sdiv(pos_sent_all_sum, m_sent_all):.4f}, "
            f"neg_rate(all): {sdiv(neg_sent_all_sum, m_sent_all):.4f}, "
        ...
        if sr.size > 0:
            ...
            sent_arr = np.asarray(sentence_counts, dtype=np.float64)
            nz_steps_mean = float(dec_arr_after.mean()) if dec_arr_after.size else 0.0
            len_mean = float(sent_arr.mean()) if sent_arr.size else 0.0
        else:
