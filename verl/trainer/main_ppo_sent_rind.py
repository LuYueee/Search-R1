    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if valid_response_length > 0:
                ...
                v_prev = float(reward_tensor[i, last_pos].item())
                ...
                if abs(v_prev) <= eps and abs(vs) > eps:
                    dec_cnt += 1
                elif abs(v_prev) > eps and abs(vs) <= eps:
        ...
        print(
            ...
            f"/max: {step_max:.4f}, /pos_rate(token): {pos_rate_token:.4f}, /neg_rate(token): {neg_rate_token:.4f}"
        )

        # sentence-level positive/negative rates (with and without zero sentences)
        ...
        sample_returns = (reward_tensor * resp_mask).sum(dim=1)
