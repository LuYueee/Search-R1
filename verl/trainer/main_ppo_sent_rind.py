    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            for pos, val in rewards:
                if pos < valid_response_length:
                    ...
                    if val == 2:
                        ...
                        tn += 1
                    if pos == valid_response_length - 1:
                        last_step_val = val
            ...
            if valid_response_length > 0:
                ...
                last_step_vals.append(last_step_val)
        ...
        # Compute step-level statistics and per-sample aggregates without extra tensor copies
        ...
        # sums and squared sums from decision rewards and episode scores
        S_dec = 2 * tp + 1 * tn - 1 * fp - 2 * fn
        SS_dec = 4 * (tp + fn) + 1 * (tn + fp)

        s_list = np.array([self.lambda_episode * s for s in all_scores], dtype=np.float64)
        S_ep = float(s_list.sum())
        SS_ep = float((s_list ** 2).sum())
        v_last = np.array(last_step_vals, dtype=np.float64)
        Cross = float((2.0 * v_last * s_list).sum())

        S_total = float(S_dec + S_ep)
        SS_total = float(SS_dec + SS_ep + Cross)
        ...
        for v, s in zip(v_last, s_list):
            ...
            if vs > 0:
                ...
                neg_dec += 1
        pos_rate = (pos_dec / N_total) if N_total > 0 else 0.0
        neg_rate = (neg_dec / N_total) if N_total > 0 else 0.0

        valid_vals = reward_tensor[resp_mask.bool()]
        step_min = float(valid_vals.min().item()) if valid_vals.numel() else 0.0
        step_max = float(valid_vals.max().item()) if valid_vals.numel() else 0.0
        ...
        if sample_returns.numel() > 0:
            return_mean = float(sample_returns.mean().item())
            return_std = float(sample_returns.std().item())
            return_min = float(sample_returns.min().item())
            return_max = float(sample_returns.max().item())
            nz_steps_mean = float(nonzero_steps.float().mean().item())
            len_mean = float(sample_lengths.float().mean().item())
        else:
