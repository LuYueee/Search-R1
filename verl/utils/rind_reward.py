...
def compute_sentence_end_rewards(rind_calc, tokenizer, generated_tokens_ids, attn_full, entropies_full, theta=1.2, solver='max'):
    """Return a list of (token_idx, reward) for each sentence in the sequence.

    Only the worker with ``rank==0`` prints debug information to avoid
    interleaved logs across distributed processes.
    """
    debug = False
    try:
