...
class RINDCalculator:
    """Utility for computing RIND score for generated text."""

    def __init__(self, model: Optional[torch.nn.Module], tokenizer) -> None:
        self.model = model
        self.tokenizer = tokenizer
        # tokenizer specific space token
        if model is not None and getattr(model.config, "model_type", "") == "llama":
            self.space_token = "▁"
        else:
    ...
    # -------------- core computation -------------------
    def compute_rind_for_generation(self, generation_outputs: SimpleNamespace, generated_tokens_ids: torch.Tensor, solver: str = "max"):
        ...
        # attention from generation or recompute
        if getattr(generation_outputs, "attentions", None) is not None:
            ...
            raise ValueError("Attention information required for RIND computation")
...
def assign_rind_rewards_for_generated_text(
    ...
    tokenizer,
    generation_outputs: SimpleNamespace,
    generated_tokens_ids: torch.Tensor,
    theta: float,
    em_sparse_value: float,
    debug: bool = False,
) -> torch.Tensor:
    """Compute sentence-level rewards and assign to token positions."""

    resp_text = tokenizer.decode(generated_tokens_ids, skip_special_tokens=True)
    doc = rind_calc.nlp(resp_text)
    sentences = [sent.text.strip() for sent in doc.sents if sent.text.strip()]

    # collect spans of tags to skip
    skip_spans: List[List[int]] = []
    ...
    for sent in sentences:
        start_pos = resp_text.find(sent, search_offset)
        if start_pos == -1:
            continue
        ...
        mini_atts = [layer[:, token_idxs[0]: token_idxs[-1] + 1, token_idxs[0]: token_idxs[-1] + 1]
                     for layer in generation_outputs.attentions]
        mini_out = SimpleNamespace(scores=tuple(mini_scores), attentions=tuple(mini_atts))
    ...
    # add final EM reward on the last token
    if len(token_scores) > 0:
        token_scores[-1] += float(em_sparse_value)
