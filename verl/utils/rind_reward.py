...
class RINDCalculator:
    ...
    def compute_rind_from_logits_attn(self, logits, attn_tensor, generated_tokens_ids, entropies=None, solver='max'):
        ...
        Args:
            ...
            entropies: Optional iterable of precomputed entropy values length L
        """
...
def compute_sentence_end_rewards(rind_calc, model, tokenizer, generated_tokens_ids, theta=1.2, solver='max', debug=False):
    ...
    logits_full = out.logits[0].float().cpu()
    attn_full = out.attentions[-1][0].float().cpu()
    probs = softmax(logits_full.numpy(), axis=-1)
    entropies_full = -np.sum(probs * np.log(probs + 1e-10), axis=-1)
    ...
    for sent in sentences:
        ...
        sub_logits = logits_full[start_idx: end_idx + 1]
        sub_attn = attn_full[start_idx: end_idx + 1, start_idx: end_idx + 1]
        sub_ents = entropies_full[start_idx: end_idx + 1]
        sent_ids = generated_tokens_ids[start_idx: end_idx + 1]
        rind_list = rind_calc.compute_rind_from_logits_attn(sub_logits, sub_attn, sent_ids, entropies=sub_ents, solver=solver)
