...
class RINDCalculator:
    """Compute RIND score for generated tokens.

    The calculator can either use an external model to obtain attentions or
    directly reuse the attentions passed in ``generation_outputs``.  This design
    allows the RL training loop to avoid loading another copy of the model –
    attentions and logits are gathered once during rollout and reused here.
    """

    def __init__(self, model, tokenizer):
        self.model = model
        self.tokenizer = tokenizer
        self.config = getattr(model, "config", SimpleNamespace(model_type=""))

        # token used to indicate a blank when merging subwords
    ...
    # ---------------- util functions -----------------
    def is_content_word(self, token_str: str) -> int:
    ...
    # ---------------- main rind calculation -----------------
    def compute_rind_for_generation(self, generation_outputs, generated_tokens_ids, solver: str = "max"):
        gen_tokens = self.tokenizer.convert_ids_to_tokens(generated_tokens_ids)
        gen_len = len(gen_tokens)

        # scores -> logits -> entropy
        scores = generation_outputs.scores  # tuple of (1, vocab)
        ...
        # attentions
        if getattr(generation_outputs, "attentions", None) is not None:
            last_layer_attn = generation_outputs.attentions[-1]
        else:
            if self.model is None:
                raise ValueError("No model provided to compute attentions")
...
def assign_rind_rewards_for_generated_text(
    rind_calc: RINDCalculator,
    tokenizer,
    generation_outputs,
    generated_tokens_ids: torch.Tensor,
    reward_tensor_row: torch.Tensor,
    theta: float = 1.2,
    em_sparse_value: float = 0.0,
    debug: bool = False,
):
    """Fill ``reward_tensor_row`` with step-wise rewards based on RIND.

    ``reward_tensor_row`` is modified in-place and should have length equal to
    the valid response length.  At the end we also add ``em_sparse_value`` to
    the final token to retain the original task reward.
    """
    resp_text = tokenizer.decode(generated_tokens_ids, skip_special_tokens=True)
    doc = rind_calc.nlp(resp_text)
    sentences = [sent.text.strip() for sent in doc.sents if sent.text.strip()]

    # collect spans for tags we should skip
    skip_spans = []
    ...
    token_scores = torch.zeros_like(reward_tensor_row)
    ...
    for sent in sentences:
        ...
        if (any(f"<{tag}" in sent for tag in ("search", "information", "answer")) or
            ...
            if debug:
                print(f"Skip tagged sentence: {sent}")
            continue
        ...
        mini_attn = [layer[:, token_idxs[0]: token_idxs[-1] + 1, token_idxs[0]: token_idxs[-1] + 1]
                     for layer in getattr(generation_outputs, "attentions", [])]
        mini_out = SimpleNamespace(scores=mini_scores, attentions=mini_attn)
        ...
        if M > theta:
            reward = 2.0 if action == "SEARCH" else -2.0
        else:
            reward = 1.0 if action in ("CONTINUE_THINK", "ANSWER") else -1.0

        token_scores[token_idxs[-1]] = reward
        if debug:
            print(f"Sentence: {sent}\n  MaxRIND={M:.4f}, Action={action}, Reward={reward}")

    token_scores[-1] += em_sparse_value
    reward_tensor_row[:token_scores.shape[0]] = token_scores
